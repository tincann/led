...
class LedDriver {
	...
	occupied(){
		...
		blink(this.led, COLOR.JUSTOCCUPIED, COLOR.OFF, 10, 300, 300, () => {
	...
	free(){
		this.clear();
		blink(this.led, COLOR.JUSTFREE, COLOR.OFF, 10, 500, 500, () => {
			this.led.setColor(COLOR.FREE);
		});
	}
...
function fadelist(led, colors, duration, cb){
	if(colors.length == 0){
		cb();
		return;
	}
	fade(led, colors.shift(), duration, () => {
		fadelist(led, colors, duration, cb);
	});
}
