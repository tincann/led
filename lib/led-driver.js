...
var COLOR = {
	...
	JUSTOCCUPIED: 	0xFFA500, // orange
	NOMOVEMENT: 	0x0000FF  // blue
}
...
class LedDriver {
	...
	occupied(){
		...
		fadeblink(this.led, COLOR.JUSTOCCUPIED, COLOR.OCCUPIED, 10, 300, 300, () => {
			fade(this.led, COLOR.OCCUPIED, 300);
		});
	}
	...
	noMovement(timeout){
		this.clear();
		fade(this.led, COLOR.NOMOVEMENT, timeout);
	}

	redetected(){
		this.clear();
		fade(this.led, COLOR.OCCUPIED, 100);
	}

	//niet meer bezet
	free(){
		this.clear();
		fadeblink(this.led, COLOR.FREE, COLOR.OFF, 10, 500, 500, () => {
			fade(this.led, COLOR.FREE, 500);
		});
	}
